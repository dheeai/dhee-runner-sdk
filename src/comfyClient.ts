...
export interface RunOpts {
  signal?: AbortSignal;
  /**
   * Budget for waiting on the JOB to finish (ms). Default 10 min.
   *
   * Not the total wall-clock ceiling: once a cloud job reports completion,
   * output resolution gets its own ~90 s budget on top of this, because
   * Comfy Cloud's history_v2 has been observed lagging the status endpoint by
   * 30–120 s under load. So a cloud call can run up to roughly
   * `timeoutMs + 90 s`. `signal` is the hard stop.
   */
  timeoutMs?: number;
  ...
  const collectFrom = (container: unknown) => {
    ...
    for (const key of OUTPUT_KEYS) {
      ...
      for (const item of list as Array<Record<string, unknown>>) {
        ...
        // Dedupe on the full Comfy identity, not the bare filename: the same
        // basename legitimately appears in different subfolders (and as both
        // 'output' and 'temp'), and keying on filename alone silently dropped
        // the second one.
        const key = `${type} ${subfolder} ${filename}`;
        if (seen.has(key)) continue;
        seen.add(key);
        outs.push({ filename, subfolder, type });
      }
    }
  };
...
/**
 * Sleep, rejecting early if `signal` aborts.
 *
 * The abort listener is removed on the normal (non-aborted) path. `{ once:
 * true }` alone is not enough: it only self-removes when the event actually
 * fires, and this is called once per poll tick against the SAME long-lived
 * signal — a 10-minute wait at the default 1500 ms interval is ~400 ticks, so
 * ~400 listeners would accumulate on a signal that never aborts.
 */
